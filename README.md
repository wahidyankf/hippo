...
`release assess --summary <path>` prints `{"accepted":true,"schemaVersion":5}` when that evidence remains inside the release envelope; rejected evidence returns exit `75` with `accepted` set to `false`.
