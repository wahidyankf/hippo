...
func (driver *Driver) publicCLIBindings() []contract.StepBinding {
	return []contract.StepBinding{
		...
		step(`^run and monitor are requested with flag values they cannot accept$`, driver.requestInvalidFlagValues),
		step(`^each exits 2 naming hippo\.args\.invalid before any payload starts$`, driver.requireInvalidFlagValuesRefused),
