...
func (driver *Driver) reservationBindings() []contract.StepBinding { //nolint:funlen,maintidx // The declarative inventory keeps every canonical reservation phrase explicit and auditable.
	...
	return []contract.StepBinding{
		...
		step(`^each request requires replanning before enqueue$`, assert("reservation floors")),
		step(`^a caller asking for memory below the floor sees exit 125 naming hippo\.policy\.replan-required before enqueue$`, driver.requireFloorMemoryReplansAtBoundary),
		step(`^a caller asking for a negative CPU count sees exit 2 naming hippo\.args\.invalid before enqueue$`, driver.requireNegativeCPUIsUsageMistakeAtBoundary),
		...
		step(`^admission requires replanning with exit 125 naming hippo\.policy\.replan-required instead of waiting$`, assert("impossible reservation")),
		...
		step(`^the waiter remains FIFO head through its deadline and is deferred with exit 124 naming hippo\.limit\.capacity-deferred$`, assert("temporary exhaustion")),
		step(`^the deferral writes a never-started receipt even when the wait's budget is nearly spent$`, requireNearlySpentBudgetDefersOnCapacity),
		...
		step(`^it rejects with exit 125 naming hippo\.coordination\.protocol-mismatch without changing compatibility state$`, assert("exclusive bridge")),
		...
		step(`^it fails with exit 125 naming hippo\.supervision\.failed and leaves the malformed state unchanged$`, driver.requireMalformedCompatibilityDeferredV04),
		...
		step(`^admission reports exit 125 naming hippo\.coordination\.protocol-mismatch without mutation and the diagnostic reports private recovery guidance$`, driver.requireUnsupportedCompatibilityV04),
		...
		step(`^it fails with exit 125 naming hippo\.supervision\.failed and leaves the service record unchanged$`, driver.requireMalformedServiceCompatibilityV04),
		...
		step(`^admission fails with exit 125 naming hippo\.supervision\.failed and preserves the session inventory with private recovery guidance$`, assert("unreadable compatibility inventory")),
		...
		step(`^admission fails with exit 125 naming hippo\.supervision\.failed without writing a reservation marker or changing heavy state$`, assert("failed stale heavy cleanup")),
		...
		step(`^both reject with exit 125 naming hippo\.coordination\.protocol-mismatch without changing the marker$`, assert("future coordination marker")),
		...
		step(`^both report exit 125 naming hippo\.coordination\.protocol-mismatch without changing the ledger$`, assert("future reservation ledger")),
		...
		step(`^it exits 125 naming hippo\.coordination\.protocol-mismatch before enqueue or child launch$`, assert("schema three legacy owner")),
		step(`^schema two reservation coordination and a resource tier with an admission wait$`, prepare("tier with admission wait", requireTierRefusesAdmissionWait)),
		step(`^tiered admission is requested$`, driver.exerciseReservationScenarioV04),
		step(`^it exits 2 naming hippo\.args\.invalid before enqueue or child launch$`, assert("tier with admission wait")),
		...
		step(`^a peer briefly holds the shared coordination lock while a guard activates and supervises its child$`,
			prepare("bounded activation contention", requireV072BoundedActivationContention)),
		step(`^the guard waits to activate its reservation and then samples through later contention$`, driver.exerciseReservationScenarioV04),
		step(`^activation succeeds without relaunch and later observation contention keeps healthy work running$`, assert("bounded activation contention")),
		...
		step(`^reservation takeover reports exit 125 naming hippo\.coordination\.protocol-mismatch until the compatibility child group retires$`, assert("schema one supervisor lifetime")),
		...
		step(`^the live PID record is retained and rejects takeover with exit 125 naming hippo\.coordination\.protocol-mismatch while only the positively stale record is reclaimed$`, assert("legacy schema one ownership")),
		...
		step(`^the child is reaped and the guard returns exit 124 naming hippo\.limit\.storage-blocked before release$`, driver.requireStorageOwnerSheddingV04),
		...
		step(`^transactional service and ephemeral owners at the emergency memory floor$`, prepare("emergency pressure", requireEmergencyPressureV05)),
		step(`^emergency pressure victims are elected atomically$`, driver.exerciseReservationScenarioV04),
		step(`^ephemeral and service owners are selected before transactional work$`, assert("emergency pressure")),
		step(`^the transactional stop records a started safety receipt without automatic retry$`, assert("emergency pressure")),
		...
		step(`^schema five reports requested allocated waited and peak totals without private inputs$`, assert("schema four summary")),
		...
		step(`^schema five reservation status aggregates the queued demand$`, driver.exerciseReservationScenarioV04),
		...
		step(`^the schema five summary reports the highest observed owner count$`, driver.requireLifetimeOwnerPeakV04),
		...
		step(`^the schema five summary includes that complete short-lived overlap$`, assert("short overlap peak")),
...
func (driver *Driver) admissionBindings() []contract.StepBinding {
	return []contract.StepBinding{
		...
		step(`^admission is storage blocked with exit 124 naming hippo\.limit\.storage-blocked$`, driver.requireStorageBlocked),
		step(`^a strict transactional task that does not fit its requested profile$`, driver.strictTransaction),
		step(`^admission requires replanning with exit 125 naming hippo\.policy\.replan-required$`, driver.requireReplan),
	}
}
...
func (driver *Driver) executionBindings() []contract.StepBinding {
	return []contract.StepBinding{
		...
		step(`^every compatibility owner is rejected with exit 125 naming hippo\.coordination\.protocol-mismatch$`, driver.requireEveryCoordinationOwnerDeferred),
		...
		step(`^it stays registered once and reports the admitted child's own exit code$`, driver.requireRetriedThenAdmitted),
		...
		step(`^it stops waiting before launch and reports the deferral as exit 124 naming hippo\.limit\.capacity-deferred$`, driver.requireDeferralSurvivesTheBudget),
		step(`^an owner waits for admission across many polls$`, func() error {
			return driver.runWaitingForAdmission(5*time.Second, 3)
		}),
		step(`^the queue and final deferral are each reported once without launching$`, driver.requireDeferralReportedOnce),
		step(`^a shared root that frees capacity onto exhausted storage$`, driver.deferringRootFreeingCapacityOntoExhaustedStorage),
		step(`^the storage notice is reported after admission without a deferral$`, driver.requireBlockedNoticeSurvivesQuieting),
		...
		step(`^the second owner is deferred with exit 124 naming hippo\.limit\.capacity-deferred$`, driver.requireDeferred),
		...
		step(`^the contender is deferred with exit 124 naming hippo\.limit\.capacity-deferred instead of failing$`, func() error {
...
func (driver *Driver) publicCLIBindings() []contract.StepBinding {
	return []contract.StepBinding{
		...
		step(`^status returns schema version 5 with profile capability and coordination evidence$`, driver.requireStatus),
		step(`^a live exclusive compatibility owner in an isolated shared root$`, driver.liveExclusiveStatusOwner),
		step(`^JSON status inspects that exclusive shared root$`, driver.inspectExclusiveStatusOwner),
		...
		step(`^current summaries from two labeled sources$`, driver.labeledHistoryV05),
		step(`^JSON history is filtered to one source for thirty days$`, driver.filteredHistoryV05),
		step(`^only the matching privacy-safe summary is returned$`, driver.requireFilteredHistoryV05),
		step(`^a history archive that is not valid gzip$`, driver.corruptHistoryArchive),
		...
		step(`^command groups are requested with an unknown or missing subcommand$`, driver.requestCommandGroupMistakes),
		step(`^each exits 2 naming hippo\.args\.invalid with its usage on stderr and nothing on stdout$`, driver.requireCommandGroupMistakesRefused),
		step(`^commands are requested with flag values they cannot accept$`, driver.requestInvalidFlagValues),
		step(`^(?:each|it) exits 2 naming hippo\.args\.invalid with only its diagnostic before any payload starts$`, driver.requireInvalidFlagValuesRefused),
		step(`^run asks to wait for admission under schema one exclusive coordination$`, driver.requestSchemaOneAdmissionWait),
		step(`^run guards a missing command whose own arguments ask for JSON output and colour$`, driver.requestChildArgumentsThatLookLikeGlobalFlags),
		step(`^HIPPO reports its failure as one plain diagnostic line without a failure body$`, driver.requireChildArgumentsIgnored),
		step(`^release monitoring is refused with its raw output flag set to json$`, driver.requestReleaseMonitorRawOutputJSON),
		...
		step(`^release summary assessment is requested through the command line$`, driver.requestRejectedAssessment),
		step(`^it exits 124 naming hippo\.limit\.release-envelope-exceeded, not retryable, and says the evidence was rejected$`, driver.requireRejectionNamed),
		step(`^run is requested from a directory whose identity file has an unknown field$`, driver.requestInvalidIdentityFile),
		step(`^a release summary file that is not a summary$`, driver.unusableSummary),
		step(`^it exits 125 naming hippo\.evidence\.unreadable with nothing on standard output$`, driver.requireUnusableSummaryRefused),
		step(`^it exits 125 naming hippo\.identity\.invalid and the identity file before any payload starts$`, driver.requireInvalidIdentityNamed),
		...
		step(`^every command rejects the unexpected argument with its own usage$`, driver.requireOperandFreeCommandsRejected),
		...
		step(`^the command exits 2 naming hippo\.args\.invalid for the missing generic health URL$`, driver.requireMissingHealthURL),
		step(`^release monitor inputs with (a malformed health URL|a malformed routed origin|no routed origin|no output path|no summary path|no deployment root|a negative duration|an out-of-range service port)$`, driver.releaseMonitorUsageMistake),
...
func (driver *Driver) releaseBindings() []contract.StepBinding {
	return []contract.StepBinding{
		...
		step(`^a release host whose (.+)$`, driver.releaseCheckHost),
		step(`^release admission is checked through the command line$`, driver.checkReleaseThroughCLI),
		step(`^it exits (\d+) naming (\S+) in one diagnostic line that says (.+)$`, driver.requireOneLineReleaseCheckFailure),
...
func (driver *Driver) qualityGateBindings() []contract.StepBinding {
	return []contract.StepBinding{
		...
		step(`^one commit-message gate binds the canonical hook message and reviewed pull request range$`, driver.requireConventionalCommits),
		step(`^the quality gate runs on pull requests rather than pushes$`, driver.requirePullRequestOnlyGate),
		step(`^stable lifecycle wiring is inspected$`, driver.inspectDocumentationGate),
		step(`^the hooks and pull request caller dispatch only closed v0.4 surfaces$`, driver.requireSharedDocumentationCheck),
		step(`^the declared main and pull request runners retain every owned policy validator$`, driver.requirePinnedValidatorsRun),
		step(`^the harness declaration owns non-overlapping Claude Codex and OpenCode projections$`, driver.requireThreeHarnessProfiles),
		step(`^the pre-commit surface invokes staged formatting for supported files$`, driver.requireStagedFormatting),
		step(`^the pre-push surface binds one typed range and invokes the direct quick gate without Nx$`, driver.requirePushQuickGate),
		...
		step(`^a compiled guarded run ends in exit 124 naming hippo\.limit\.capacity-deferred with a never-started receipt$`, driver.documentedCapacityDeferral),
		step(`^a compiled guarded run ends in another exit or without the hippo\.limit\.capacity-deferred reason or receipt$`, driver.otherRefusals),
		step(`^a compiled guarded run ends in protocol mismatch exit 125$`, driver.protocolMismatchRefusal),
