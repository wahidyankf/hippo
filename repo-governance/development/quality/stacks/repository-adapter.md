...
| `shell`  | adapted | two declared interpreters, and no static-analysis gate yet, as recorded below                 |
...
| `shell-scripts.md`      | interpreter           | deviation: POSIX `sh` with `set -eu`; Bash only in the adopted scanner | the bootstrap wrappers run before any toolchain on macOS and Linux; the scanner stays as adopted |
| `shell-standards.md`    | static analysis       | gap: not yet a gate; `shfmt` formats every script                      | wiring an analyser is outside this adoption; the standard is not weakened                        |
