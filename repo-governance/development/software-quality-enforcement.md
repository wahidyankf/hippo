...
The pinned RHINO release answers six independent questions: the configuration parses, word budgets hold, directory maps match the tree, internal links resolve, diagrams stay legible, and the harness adapter tree matches the canon. All six are declared gates on the `pre-push`, `pull-request`, and `main` publication surfaces.
