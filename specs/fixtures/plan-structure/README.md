...
This copy is owned here. Nothing pins it to `ose-rules`, and nothing obliges this repository to notice that the catalog
moved — see [Fixture Corpora](../README.md).

## Changing the Corpus

Adding a case is normal. Editing one is not, unless the rule it encodes changed.

Any change regenerates `SHA256SUMS` in the same change, so the integrity check keeps describing what is actually here.
