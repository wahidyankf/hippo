...
After this check, `main` moved to `5bc7150` and the three commits were replayed onto it for a replacement pull
request. Re-reading the replayed branch changed no step above: the test diff is identical, and the only new difference
is the done index, which now lists the shell static-analysis plan beside this one.
